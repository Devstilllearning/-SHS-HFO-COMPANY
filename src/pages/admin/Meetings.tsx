...
export default function AdminMeetings() {
  ...
  const filteredMeetings = meetings.filter(m => {
    const matchesSearch = 
      ...
      m.memberData?.name?.toLowerCase().includes(search.toLowerCase()) ||
      m.memberRole?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="flex min-h-screen bg-bg-light">
      <Sidebar />
      <main className="flex-1 ml-[280px] h-full flex flex-col p-10">
        <header className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-serif font-bold text-bg-dark">All Meetings</h1>
            <p className="text-text-muted mt-1">Global oversight of all scheduled coordination.</p>
          </div>
        </header>
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-8">
          <div className="flex-1 relative">
            ...
            <input 
              type="text"
              placeholder="Search by guest, company, or team member..."
          ...
          <div className="flex items-center space-x-4">
            <div className="relative border border-gray-100 rounded-[12px] bg-white">
              <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-purple" />
              <select 
                className="pl-10 pr-8 py-3 bg-transparent focus:outline-none font-bold text-sm text-bg-dark appearance-none cursor-pointer"
                value={filter}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-12 w-full">
                    ...
                    <div className="p-6">
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <h3 className="font-bold text-lg text-bg-dark">{meeting.guestName} <span className="font-medium text-sm text-gray-500">({meeting.companyName})</span></h3>
                            <div className="flex items-center text-[10px] font-bold text-text-muted uppercase tracking-widest mt-0.5">
                              ...
                              <span>Target: {meeting.memberData?.name || meeting.memberRole?.replace('_', ' ') || 'Member'}</span>
                            </div>
                          </div>
                        </div>
              ...
              {!loading && filteredMeetings.length === 0 && (
                <div className="xl:col-span-2 py-24 text-center bg-white rounded-[16px] border border-dashed border-gray-200">
                  ...
                  <h3 className="text-xl font-serif font-bold text-bg-dark">No Meetings</h3>
                  <p className="text-text-muted text-sm px-12">No meetings found matching your filters.</p>
                </div>
              )}
            </div>
          )}
